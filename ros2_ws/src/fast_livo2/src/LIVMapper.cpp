...
void LIVMapper::readParameters(rclcpp::Node::SharedPtr nh)
{
  ...
  GET_PARAM(nh, "vio.normal_en", normal_en, true);
  GET_PARAM(nh, "vio.inverse_composition_en", inverse_composition_en, false);
  GET_PARAM(nh, "vio.max_iterations", max_iterations, 5);
  GET_PARAM(nh, "vio.img_point_cov", IMG_POINT_COV, 100);
  GET_PARAM(nh, "vio.raycast_en", raycast_en, false);
  GET_PARAM(nh, "vio.exposure_estimate_en", exposure_estimate_en, true);
  GET_PARAM(nh, "vio.inv_expo_cov", inv_expo_cov, 0.2);
  GET_PARAM(nh, "vio.grid_size", grid_size, 5);
  GET_PARAM(nh, "vio.grid_n_height", grid_n_height, 17);
  GET_PARAM(nh, "vio.patch_pyrimid_level", patch_pyrimid_level, 3);
  GET_PARAM(nh, "vio.patch_size", patch_size, 8);
  GET_PARAM(nh, "vio.outlier_threshold", outlier_threshold, 1000);

  GET_PARAM(nh, "time_offset.exposure_time_init", exposure_time_init, 0.0);
  GET_PARAM(nh, "time_offset.img_time_offset", img_time_offset, 0.0);
  GET_PARAM(nh, "time_offset.imu_time_offset", imu_time_offset, 0.0);
  GET_PARAM(nh, "time_offset.lidar_time_offset", lidar_time_offset, 0.0);
  GET_PARAM(nh, "uav.imu_rate_odom", imu_prop_enable, false);
  GET_PARAM(nh, "uav.gravity_align_en", gravity_align_en, false);

  GET_PARAM(nh, "evo.seq_name", seq_name, "01");
  GET_PARAM(nh, "evo.pose_output_en", pose_output_en, false);
  GET_PARAM(nh, "imu.gyr_cov", gyr_cov, 1.0);
  GET_PARAM(nh, "imu.acc_cov", acc_cov, 1.0);
  GET_PARAM(nh, "imu.imu_int_frame", imu_int_frame, 3);
  GET_PARAM(nh, "imu.imu_en", imu_en, false);
  GET_PARAM(nh, "imu.gravity_est_en", gravity_est_en, true);
  GET_PARAM(nh, "imu.ba_bg_est_en", ba_bg_est_en, true);

  GET_PARAM(nh, "preprocess.blind", p_pre->blind, 0.01);
  GET_PARAM(nh, "preprocess.filter_size_surf", filter_size_surf_min, 0.5);
  GET_PARAM(nh, "preprocess.hilti_en", hilti_en, false);
  GET_PARAM(nh, "preprocess.lidar_type", p_pre->lidar_type, AVIA);
  GET_PARAM(nh, "preprocess.scan_line", p_pre->N_SCANS, 6);
  GET_PARAM(nh, "preprocess.point_filter_num", p_pre->point_filter_num, 3);
  GET_PARAM(nh, "preprocess.feature_extract_enabled", p_pre->feature_enabled, false);

  GET_PARAM(nh, "pcd_save.interval", pcd_save_interval, -1);
  GET_PARAM(nh, "pcd_save.pcd_save_en", pcd_save_en, false);
  GET_PARAM(nh, "pcd_save.type", pcd_save_type, 0);
  GET_PARAM(nh, "image_save.img_save_en", img_save_en, false);
  GET_PARAM(nh, "image_save.interval", img_save_interval, 1);

  GET_PARAM(nh, "pcd_save.colmap_output_en", colmap_output_en, false);
  GET_PARAM(nh, "pcd_save.filter_size_pcd", filter_size_pcd, 0.5);
  GET_PARAM(nh, "extrin_calib.extrinsic_T", extrinT, std::vector<double>());
  GET_PARAM(nh, "extrin_calib.extrinsic_R", extrinR, std::vector<double>());
  GET_PARAM(nh, "extrin_calib.Pcl", cameraextrinT, std::vector<double>());
  GET_PARAM(nh, "extrin_calib.Rcl", cameraextrinR, std::vector<double>());
  GET_PARAM(nh, "debug.plot_time", plot_time, -10);
  GET_PARAM(nh, "debug.frame_cnt", frame_cnt, 6);

  GET_PARAM(nh, "publish.blind_rgb_points", blind_rgb_points, 0.01);
  GET_PARAM(nh, "publish.pub_scan_num", pub_scan_num, 1);
  GET_PARAM(nh, "publish.pub_effect_point_en", pub_effect_point_en, false);
  GET_PARAM(nh, "publish.dense_map_en", dense_map_en, false);
...
void LIVMapper::initializeSubscribersAndPublishers(rclcpp::Node::SharedPtr nh, image_transport::ImageTransport &it) 
{
  ...
  if (img_en) {
    sub_img = nh->create_subscription<sensor_msgs::msg::Image>(
...
{    
  ...
  if (pose_output_en) 
  {
    ...
    if (!pos_opend) 
    {
      ...
      pos_opend = true;
      if (!evoFile.is_open()) RCLCPP_ERROR(rclcpp::get_logger("livo"), "open fail\n");
    } 
    else 
    {
...
void LIVMapper::standard_pcl_cbk(const sensor_msgs::msg::PointCloud2::ConstSharedPtr msg)
{
  ...
  {
    RCLCPP_ERROR(rclcpp::get_logger("livo"), "lidar loop back, clear buffer");
    lid_raw_data_buffer.clear();
  }
  // RCLCPP_INFO(rclcpp::get_logger("livo"), "get point cloud at time: %.6f", rclcpp::Time(msg->header.stamp).seconds());
...
void LIVMapper::imu_cbk(const sensor_msgs::msg::Imu::ConstSharedPtr msg_in)
{
  ...
  {
    double t = rclcpp::Time(msg->header.stamp).seconds() - imu_time_offset;
    msg->header.stamp.sec = static_cast<int32_t>(t);
    msg->header.stamp.nanosec = static_cast<uint32_t>((t - static_cast<int64_t>(t)) * 1e9);
  }
  ...
  if (ros_driver_fix_en) timestamp += std::round(last_timestamp_lidar - timestamp);
  {
    ...
    msg->header.stamp.nanosec = static_cast<uint32_t>((timestamp - static_cast<int64_t>(timestamp)) * 1e9);
  }
...
void LIVMapper::img_cbk(const sensor_msgs::msg::Image::ConstSharedPtr msg_in)
{
  ...
  RCLCPP_INFO(rclcpp::get_logger("livo"), "Get image, its header time: %.6f", msg_header_time);
  if (last_timestamp_lidar < 0) return;

  if (msg_header_time < last_timestamp_img)
  {
    RCLCPP_ERROR(rclcpp::get_logger("livo"), "image loop back. \n");
    return;
  }
  ...
  {
    RCLCPP_WARN(rclcpp::get_logger("livo"), "Image need Jumps: %.6f", img_time_correct);
    mtx_buffer.unlock();
    sig_buffer.notify_all();
    return;
  }
  ...
  // RCLCPP_INFO(rclcpp::get_logger("livo"), "Correct Image time: %.6f", img_time_correct);
...
bool LIVMapper::sync_packages(LidarMeasureGroup &meas)
{
  ...
  case ONLY_LIO:
  {
    ...
    { // waiting imu message needs to be
      ...
      // RCLCPP_ERROR(rclcpp::get_logger("livo"), "out sync");
      return false;
    }
    ...
    // RCLCPP_INFO(rclcpp::get_logger("livo"), "ONlY HAS LiDAR and IMU, NO IMAGE!");
  ...
  case LIVO:
  {
    ...
    case VIO:
    {
      ...
      if (img_capture_time < meas.last_lio_update_time + 0.00001)
      {
        ...
        RCLCPP_ERROR(rclcpp::get_logger("livo"), "[ Data Cut ] Throw one image frame! \n");
        return false;
      }

      if (img_capture_time > lid_newest_time || img_capture_time > imu_newest_time)
      {
        // RCLCPP_ERROR(rclcpp::get_logger("livo"), "lost first camera frame");
...
void LIVMapper::publish_odometry(const rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr pubOdomAftMapped)
{
  ...
  odomAftMapped.header.stamp = nh_->now();
  ...
  transformStamped.transform.translation.x = _state.pos_end(0);
  transformStamped.transform.translation.y = _state.pos_end(1);
  transformStamped.transform.translation.z = _state.pos_end(2);
  transformStamped.transform.rotation = geoQuat;
