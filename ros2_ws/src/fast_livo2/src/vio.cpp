...
void VIOManager::getWarpMatrixAffineHomography(const vk::AbstractCamera &cam, const V2D &px_ref, const V3D &xyz_ref, const V3D &normal_ref,
                                                  const Sophus::SE3d &T_cur_ref, const int level_ref, Matrix2d &A_cur_ref)
{
...
void VIOManager::getWarpMatrixAffine(const vk::AbstractCamera &cam, const Vector2d &px_ref, const Vector3d &f_ref, const double depth_ref,
                                        const Sophus::SE3d &T_cur_ref, const int level_ref, const int pyramid_level, const int halfpatch_size,
                                        Matrix2d &A_cur_ref)
{
  ...
  for (int i = 0; i < length; i++)
  {
    if (grid_num[i] == TYPE_MAP)
    {
      ...
      if (normal_en)
      {
        ...
        Sophus::SE3d T_cur_ref = new_frame_->T_f_w_ * ref_ftr->T_f_w_.inverse();
...
void VIOManager::updateVisualMapPoints(cv::Mat img)
{
  ...
  int update_num = 0;
  Sophus::SE3d pose_cur = new_frame_->T_f_w_;
  for (int i = 0; i < total_points; i++)
  {
    ...
    // Step 2: delta_pose
    Sophus::SE3d pose_ref = last_feature->T_f_w_;
...
void VIOManager::updateFrameState(StatesGroup state)
{
  ...
  Rcw = Rci * Rwi.transpose();
  Pcw = -Rci * Rwi.transpose() * Pwi + Pci;
  new_frame_->T_f_w_ = Sophus::SE3d(Rcw, Pcw);
}
