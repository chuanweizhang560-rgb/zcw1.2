...
{
  GET_PARAM(nh, "publish.pub_plane_en", voxel_config.is_pub_plane_map_, false);
  
  GET_PARAM(nh, "lio.max_layer", voxel_config.max_layer_, 1);
  GET_PARAM(nh, "lio.voxel_size", voxel_config.max_voxel_size_, 0.5);
  GET_PARAM(nh, "lio.min_eigen_value", voxel_config.planner_threshold_, 0.01);
  GET_PARAM(nh, "lio.sigma_num", voxel_config.sigma_num_, 3);
  GET_PARAM(nh, "lio.beam_err", voxel_config.beam_err_, 0.02);
  GET_PARAM(nh, "lio.dept_err", voxel_config.dept_err_, 0.05);
  {
    std::vector<long int> tmp{5,5,5,5,5};
    try { nh->declare_parameter("lio.layer_init_num", tmp); } catch(...) {}
    nh->get_parameter("lio.layer_init_num", tmp);
    voxel_config.layer_init_num_.assign(tmp.begin(), tmp.end());
  }
  GET_PARAM(nh, "lio.max_points_num", voxel_config.max_points_num_, 50);
  GET_PARAM(nh, "lio.max_iterations", voxel_config.max_iterations_, 5);

  GET_PARAM(nh, "local_map.map_sliding_en", voxel_config.map_sliding_en, false);
  GET_PARAM(nh, "local_map.half_map_size", voxel_config.half_map_size, 100);
  GET_PARAM(nh, "local_map.sliding_thresh", voxel_config.sliding_thresh, 8);
}
...
void VoxelMapManager::pubVoxelMap()
{
  ...
  double pow_num = 0.2;
  rclcpp::WallRate loop(std::chrono::milliseconds(2));
  float use_alpha = 0.8;
