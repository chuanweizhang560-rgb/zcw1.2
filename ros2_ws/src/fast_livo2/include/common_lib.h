...
#define GET_PARAM(NH, NAME, VAR, DEFAULT) do { \
  try { NH->declare_parameter<decltype(VAR)>(NAME, DEFAULT); } catch(...) {} \
  NH->get_parameter(NAME, VAR); \
} while(0)

// ROS 1 → ROS 2 message type compatibility aliases
namespace sensor_msgs { using msg::Imu; using msg::Image; using msg::PointCloud2; }
namespace nav_msgs { using msg::Odometry; using msg::Path; }
namespace geometry_msgs { using msg::PoseStamped; using msg::Quaternion; using msg::TransformStamped; }
namespace visualization_msgs { using msg::Marker; using msg::MarkerArray; }
...
template <typename T>
auto set_pose6d(const double t, const Eigen::Matrix<T, 3, 1> &a_in, const Eigen::Matrix<T, 3, 1> &g_in, const Eigen::Matrix<T, 3, 1> &v_in, const Eigen::Matrix<T, 3, 1> &p_in,
                const Eigen::Matrix<T, 3, 3> &R_in)
{
  Pose6D rot_kp;
